...
class ContaControllerTest {
    ...
    @Test
    @DisplayName("GET /api/contas deve retornar lista vazia sem erro")
    ...
    void deveBuscarContaPorId() throws Exception {
        when(contaService.consultarPorId(1L)).thenReturn(new Conta(1L, "Pedro", 500.0));
    ...
    @Test
    @DisplayName("GET /api/contas/99 deve retornar 404 para conta inexistente")
    void deveRetornar404ParaContaInexistente() throws Exception {
        when(contaService.consultarPorId(99L)).thenThrow(new ContaNaoEncontradaException(99L));
    ...
    void deveRetornar400ParaNomeVazio() throws Exception {
    ...
    void deveRetornar400ParaSaldoNegativo() throws Exception {
        doThrow(new SaldoInvalidoException("O saldo não pode ser negativo: -10.0"))
                .when(contaService).incluirConta(any());
    ...
    @Test
    @DisplayName("POST com body malformado deve retornar 400")
    ...
    void deveLancar404AoAtualizarInexistente() throws Exception {
        doThrow(new ContaNaoEncontradaException(99L))
                .when(contaService).alterarConta(eq(99L), any());
    ...
    void deveLancar404AoExcluirInexistente() throws Exception {
    ...
    @Test
    @DisplayName("Falha inesperada no serviço deve retornar 500 sem expor detalhes internos")
    ...
    void fuzzEntradasMaliciosas(String entrada) throws Exception {
        doThrow(new SaldoInvalidoException("O nome não pode ser vazio."))
                .when(contaService).incluirConta(any());
